...
Select sum(Salary) as [TotalSalary] from Employee where Age>=40;

--33. What is the combined salary that you need to pay to all the Freshers and Programmers for 1 month
Select Sum(Salary) as [TotalSalary] from Employee where Title='Fresher' or Title='programmer';

--34. What is the combined salary that you need to pay to all the Freshers whose age is greater than 27 for 3years
Select Sum(Salary)*3*12 as [TotalSalary] from Employee where Title='Fresher' and Age>27;
...
Select * from Employee where Age in (select min(Age) from Employee where Title='Manager');
...
select FirstName,Age from Employee where (FirstName='John' or FirstName='Micheal') and (Salary between 17000 and 26000);
select FirstName,Age from Employee where (Salary between 17000 and 26000) and FirstName in (select FirstName from Employee where (FirstName='John' or FirstName='Micheal'));
...
select Title,Count(Title) as [NoOFEmployees] from Employee group by Title order by Title;
...
select Title, Avg(Salary) as [averageSalary] from Employee where Title!='Fresher' group by Title;
...
update Employee set Age=Age+1 ,Salary+=5000 where FirstName='Ginger' and LastName='Finger';
